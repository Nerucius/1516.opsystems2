...
/**
 * Create a new tree and allocate memory
 */
RBTree * tree_new()
{
...
void tree_insertNode(RBTree *tree, RBData *data){
	...
	if ((x = malloc(sizeof(*x))) == 0) {
		printf("insufficient memory (tree_insertNode)\n");
		exit(1);
	}
 ...
 */
void tree_delete(RBTree *tree, void (*deleteData)(void *data))
{
  if (tree->root != NIL)
